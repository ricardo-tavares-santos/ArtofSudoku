...
public class SudokuManager : MonoBehaviour
{
    ...
    public void LoadNewGame()
    {
        ...
        LoadGame(mode, level);
		RandomBackground.intance.setBackground();
		RandomSFX.intance.playSFX();
    }
    ...
    IEnumerator ShowGameOverPopup()
    {
		...
		//???
        int h = Mathf.FloorToInt(GameManager.GameTime / 3600f);
        int m = Mathf.FloorToInt(GameManager.GameTime / 60f) % 60;
        int s = Mathf.FloorToInt(GameManager.GameTime) % 60;	
		...
		if (PlayerPrefs.GetInt("GameMode", 0) == 0) {
			if (GameServices.IsInitialized()) {
				GameServices.ReportScore(Convert.ToInt64(final), EM_GameServicesConstants.Leaderboard_easy);
			} else {
				GameServices.Init();   
				GameServices.ReportScore(Convert.ToInt64(final), EM_GameServicesConstants.Leaderboard_easy);
			}		
			...
			if (GameServices.IsInitialized()) {
				GameServices.ReportScore(Convert.ToInt64(final), EM_GameServicesConstants.Leaderboard_medium);
			} else {
				GameServices.Init();   
				GameServices.ReportScore(Convert.ToInt64(final), EM_GameServicesConstants.Leaderboard_medium);
			}			
			...
			if (GameServices.IsInitialized()) {
				GameServices.ReportScore(Convert.ToInt64(final), EM_GameServicesConstants.Leaderboard_hard);
			} else {
				GameServices.Init();   
				GameServices.ReportScore(Convert.ToInt64(final), EM_GameServicesConstants.Leaderboard_hard);
			}
		} else if (PlayerPrefs.GetInt("GameMode", 0) == 3) {
			if (GameServices.IsInitialized()) {
				GameServices.ReportScore(Convert.ToInt64(final), EM_GameServicesConstants.Leaderboard_expert);
			} else {
				GameServices.Init();   
				GameServices.ReportScore(Convert.ToInt64(final), EM_GameServicesConstants.Leaderboard_expert);
			}			
